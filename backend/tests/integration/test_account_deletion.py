...
async def test_hard_delete_removes_all_user_scoped_rows_and_the_user(db_session, current_test_user):
    ...
    for model in (PersonalInfo, Education, Experience, Project, SkillCategory, Job, AICredential):
...
async def test_hard_delete_removes_cached_pdfs(db_session, current_test_user):
    from app.services.pdf_storage import get_pdf_storage, resume_pdf_key
