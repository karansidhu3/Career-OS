...
**ADR-018** — Evidence-backed generation v2 (2026-08-08). Long profile descriptions remain the source of truth but are compiled by Haiku 4.5 into a versioned, evidence-quoted fact bank only when the profile hash changes. Each application uses a low-cost Haiku plan to select canonical project/fact IDs, then Sonnet 4.6 writes structured bullets and a cover letter from only the selected sources. Every bullet carries fact provenance; unknown IDs and unsupported numbers are rejected. Python owns identity, dates, section structure, skills, LaTeX, and final project names. The rendered PDF must prove it is exactly one page before the job can succeed; fitting is deterministic and makes no compression LLM calls. Every provider call is recorded in `llm_calls`, with complete per-model cost stored on the job. Migration 017 adds the required audit tables/columns and must be explicitly flagged before deployment.
