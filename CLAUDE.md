...
- Email integration for notifications, digests, or marketing — pull-only product. **Exception**
  (decided during Phase 6 planning, 2026-07-01): transactional email for one
  security-critical account-lifecycle trigger — account deletion confirmation — via a
  minimal `EmailClient` seam (mirrors `LLMClient`/`PDFStorage`). Nothing beyond that
  trigger; no engagement/notification email of any kind. (Originally two triggers —
  data export ready was the other — until account data export was removed entirely,
  see ADR-017.)
...
| Hosting | Fly.io (API + in-process ARQ worker, one Docker image), Vercel (frontend) |
...
ACCOUNT: /account
  ...
  → Administrative, not content: Anthropic API key, account deletion (with a "Delete now"
    escape hatch once the 7-day grace period has started — see ADR-017)
  → No session/device management UI — see note on UserMenu below
  → No account data export — removed, see ADR-017
```
...
- 2–4 projects total, selected by the full-context model as a complementary portfolio; one-page compilation may remove the weakest project only when necessary
- A selected source's strongest job-relevant Tier 1 metric should appear; irrelevant numbers and vanity counts should not be forced into the résumé
- `backend/app/services/generation.py` is the live full-context prompt, editorial acceptance gate, template system, and one-page compiler. `generation_v2.py` and its benchmark remain for historical comparison but are not the production writer.
...
**ADR-003** — Fly.io for the backend API and its in-process ARQ worker (one Docker-based app). Tectonic is installed during the image build for server-side LaTeX→PDF compilation. Frontend on Vercel, Postgres on Neon, Redis on Upstash. Migrated off Railway (2026-07-01) for usage-based infrastructure costs. The original two-Fly-app design was consolidated later; see ADR-010.

**ADR-004** — Background/async generation. POST `/admin/jobs/generate` returns immediately (status="processing"); actual generation runs through Redis-backed ARQ in a worker embedded in the FastAPI process (see ADR-010), not inline in the request. Frontend polls every 2.5s until done. The queue boundary avoids HTTP proxy timeouts; interrupted or older-than-15-minute attempts are converted to visible retryable failures rather than polling forever.
...
**ADR-007** — Candidacy insights. After 3+ completed applications, `GET /admin/jobs/insights` deterministically counts normalized gaps already stored in recent generation metadata and returns the most repeated actionable pattern. It makes no LLM call and does not require an API key. Cached in localStorage for 1 hour on the frontend.
...
**ADR-010** — Migrated hosting from Railway to Fly.io + Vercel + Neon + Upstash (2026-07-01), cost-driven (see ADR-003). The original deployment used separate `careeros-backend` and `careeros-worker` Fly apps. It was later consolidated: `app.main` now creates the Redis enqueue pool and runs an ARQ worker inside the single `careeros-backend` process, halving compute cost while preserving the durable queue boundary. The legacy `fly.worker.toml` and empty Fly app remain historical artifacts and are not part of deployment. The backend runs one `shared-cpu-1x` machine with 1 GB memory; Tectonic and generation workloads made smaller allocations unreliable. Deploy shutdown explicitly closes ARQ, interrupted jobs are saved as retryable failures, stale `processing` rows self-heal after 15 minutes, and Fly allows a 30-second termination window for cleanup. Postgres connection strings from non-Railway providers (Neon included) include `?sslmode=require`, which asyncpg's `connect()` rejects (`ssl=` is required); `app.database._normalize()` rewrites this automatically. The Neon `careeros_app` restricted role was created manually with its own generated credential; never reuse roles or provider-suggested security secrets across services.
...
**ADR-013** — Encrypted BYO Anthropic API key, no shared fallback (Phase 3, 2026-07-01). Every operation that calls Anthropic requires the requesting user's own stored key or fails with a clear 400; candidacy insights no longer call Anthropic as of ADR-018. `settings.anthropic_api_key` was removed entirely, this is a hard cutover, not an optional path. Keys live in `ai_credentials` (`user_id`, `provider`, `encrypted_key`, `key_hint`), encrypted via `app/services/crypto.py`'s envelope encryption (`cryptography`'s `Fernet`/`MultiFernet`, so rotating `ENCRYPTION_MASTER_KEY` doesn't break decryption of rows encrypted under a previous key — `ENCRYPTION_MASTER_KEY_PREVIOUS` holds prior keys comma-separated). `POST /admin/settings/api-key` validates the submitted key with one real tiny forced-tool-call to Anthropic, billed to the submitter's own key, before storing it. This is the actual mechanism behind "CareerOS owns the workflow, users own their AI usage" — see README's "why it isn't just a resume generator."
...
**ADR-016** — Onboarding gate overhaul + multi-template resume system (2026-07-09). Reordered onboarding to key → profile → template → idle with a minimal step indicator ("Step N of 3") and deeper "why this matters" copy per step — a scoped exception to the "no gamification/progress metrics" and "not a dashboard" hard constraints, justified because this is a one-time, pre-core-loop setup gate that disappears for good once a profile clears the minimum bar, not a persistent status surface (see "What NOT to build" above). Added 6 named LaTeX resume templates (jake/crisp/modern/sharp/classic/minimal) plus a user-supplied custom preamble, all with real compiled-PDF previews (not static thumbnails) in both the onboarding picker and `/profile`'s Preferences tab, and a click-to-expand full-size modal. `/profile` itself was restructured from one continuous scroll of 7 sections into an Apple System Settings-style tab bar (Personal/Projects/Experience/Skills/Education/Preferences), each on its own screen, plus a `NoPersonalInfoYet` fallback for the case where a user reaches `/profile` before onboarding has created `personal_info` at all. Also fixed two real bugs surfaced along the way: (1) a pre-existing LaTeX bug where `\titlerule[Npt]` nested inside `\titleformat`'s own optional argument silently broke parsing whenever combined with `mathpazo`/`mathptmx` — the "Modern" template had never actually compiled successfully before this — fixed by switching to `\rule{\linewidth}{Npt}`; (2) a JIT-user-provisioning race in `get_current_user` (`clerk_auth.py`) — the onboarding gate fires two concurrent requests (`Promise.all([getProfile, getApiKeyStatus])`) on a user's very first sign-in, which could both see no existing row and both try to `INSERT`, with the loser hitting the `clerk_user_id` unique constraint and 500ing — the frontend treated that 500 as "transient error, fail open," dropping a brand-new user with no profile and no API key straight onto the full generation screen. Fixed by recovering from the `IntegrityError` server-side and retrying once client-side before falling open. Separately, closed a related loophole: a user could reach `Generate` with zero experience and zero projects — nothing for the model to write from — and burn their own Anthropic tokens on an empty result. `_has_generatable_content` (`jobs.py`) now rejects `/admin/jobs/generate` and `/admin/jobs/{id}/regenerate` with a 400 in that case, checked client-side first so the common case never hits the network, with Generate staying fully clickable (no disabled state) and the JD text preserved on rejection.

**ADR-017** — Removed account data export; added an immediate "Delete now" option to account deletion (2026-07-10). Account data export (Phase 6) — the async zip-of-everything feature with its own `AccountExport` model, ARQ worker job, R2 storage, and "your export is ready" email — was cut entirely: not needed for this product. Removed the model, service (`account_export.py`), worker job (`run_export_job`), all three `/admin/account/export*` endpoints, the frontend `AccountDataExport` component, and the corresponding `api.ts` methods; `migrations/016_drop_account_exports.sql` drops the now-orphaned table (not yet run — flagged per the DB-migration rule). The `EmailClient` seam and its one-transactional-email carve-out survive with a single remaining trigger (account deletion confirmation) rather than two. Separately, account deletion's 7-day grace period previously had no way to skip the wait — only "Cancel deletion." Added `POST /admin/account/delete/now`, reachable only once a deletion is already scheduled (an acceleration of an in-progress deletion, not a parallel path around the confirmation step that starts one) and gated behind its own typed "DELETE" confirmation, since it is strictly more consequential than the original request (no more undo window once it runs). Calls `hard_delete_user` directly inside the request — `get_current_user` has already set the RLS GUC on the session, the same mechanism `app.worker`'s hourly cron sweep sets up manually for its own out-of-request-context session. The frontend signs the user out and redirects to `/` immediately on success, since there is no account left to show a confirmation screen on.

**ADR-018** — Evidence-backed generation v3.6 (2026-08-08, evolved through 2026-08-17). Long profile descriptions remain the source of truth and are split locally into a versioned, evidence-quoted fact bank when the profile hash changes; profile compilation no longer makes an AI call. Each application makes one prompt-cached Sonnet 4.6 call that plans and writes structured bullets, the cover letter, fit, matches, and gaps together. Project selection is deterministic before that call: candidates are ranked 35% exact role/technology relevance, 25% proof of the job's main responsibilities, 20% outcome/user value, 15% engineering depth, and 5% marginal distinctiveness from projects already selected, so the final set covers the role rather than merely collecting the most complex projects. Three projects is the default; a fourth is permitted only when a conservative pre-generation layout estimate predicts room. Every bullet carries fact provenance; unknown IDs, unsupported technologies, acronyms, and numbers are rejected. Cover-letter paragraph two is a focused case study of exactly one project: one problem, one decision, one result, no more than two metrics, and no test-suite inventory. Strategic analysis is stored as structured match and gap evidence; Python renders the final GOOD FIT, GAPS, and IMPROVEMENT PLAN language, validates each deliverable, and supplies observable completion signals. Each completed generation now stores a self-contained offline evaluation artifact. The six-role benchmark under `backend/benchmarks/` scores grounding, relevance, clarity, technical depth, ownership, metrics, cover focus, density, grammar, cost, and repair use; its acceptance gate rejects quality regressions and unjustified cost growth without making another AI call. Grounded literal evidence is attempted locally before any paid sectional repair, and an invalid optional fourth project is dropped for free. Python owns identity, dates, section structure, skills, project selection, LaTeX, one-page fitting, and final project names. Every provider call is recorded in `llm_calls`, with complete per-model cost stored on the job. Migration 017 added the required audit tables/columns.

**ADR-019** — Temporary original-prompt rollback (2026-08-18). The active worker was switched from `generation_v2.generate_materials_v2` back to the original full-context `generation.generate_materials` pipeline after v3.6 recovery emitted literal profile fragments as finished bullets (including a one-word project bullet) and assembled a malformed fallback cover letter. The evidence-backed v3.6 implementation and benchmark remain in the repository but are inactive while recovery and semantic acceptance are redesigned. The rollback was labeled `original-prompt-v1`; ADR-020 immediately evolves that restored baseline into the active prompt. This deliberately restores the original pipeline's higher token use and model-driven project selection in exchange for the materially stronger editorial output; its token-derived API cost remains visible, but v3.6's complete per-call audit metadata is unavailable during the rollback.

**ADR-020** — Ultimate prompt v1 (2026-08-18). The active full-context generator now treats truth as the boundary and maximum defensible leverage as the objective. Claude receives the complete profile and JD, owns the positioning thesis and model-led 2–4 project portfolio, and may synthesize multiple supported facts from one source into stronger recruiter-facing language. Requirements are classified internally as direct evidence, transferable evidence, or genuinely absent; adjacent stacks prove underlying responsibilities without being misrepresented as exact tools. The prompt preserves the original metric hierarchy, company classification, recruiter-sale/engineering-proof bullet structure, descriptors, ATS mirroring, and self-review, while adding portfolio-level coverage, non-redundancy, a density floor, natural supported-keyword distribution, and an explicit ban on raw profile sentences, fragments, passive stack inventories, and gap language inside application documents. New jobs are labeled `ultimate-prompt-v1`. Quality remains ahead of cost; prompt/profile caching and avoiding unnecessary regeneration are the approved cost controls, not smaller context or degraded fallback writing.

**ADR-021** — Full-context editorial acceptance gate (2026-08-20). Production remains on the restored, quality-first full-context generator rather than v3.6's fact-bank writer. A deterministic gate now inspects the generated LaTeX body before compilation and rejects the concrete defects that escaped v3.6: fragments and one-word bullets, passive project/technology inventories, unsupported numeric claims, missing punctuation, malformed bullet counts, and semantically repetitive bullets within one entry. A failed draft receives one evidence-aware résumé-body rewrite using the complete profile, JD, selected project set, draft, and exact gate failures. The rewrite must pass the same gate before compilation; one-page compression is checked again afterward. If either acceptance pass fails, the job fails visibly instead of storing an embarrassing fallback document. New jobs are labeled `ultimate-prompt-v2-quality-gated`, and generation metadata records whether the repair was required. Literal fact-bank fallback prose is no longer on the production path.

**ADR-022** — Deterministic one-page layout rescue (2026-08-26). The full-context generator still permits at most two paid Claude compression calls when a compiled résumé exceeds one page. If the second result remains multi-page, CareerOS now performs free, compilation-verified reductions instead of immediately failing: Skills categories and their contents are generated most-to-least JD-relevant and removed from the end first; the Skills section is removed only after its final row remains insufficient; then the lowest-ranked project may be removed while preserving at least two projects. Every deterministic edit is recompiled before acceptance, the final rendered project names replace the original selection metadata so the UI stays truthful, and the applied rescue actions are recorded in generation metadata. No template geometry or font size changes, PDF page cropping, or additional provider calls are used.
...
**ADR-024** — Deterministic editorial overflow recovery (2026-08-30). The full-context quality repair still receives one paid attempt, but its prompt now targets 16–24 words and caps its own output at 30 words rather than treating the validator's 38-word emergency ceiling as the writing target. If that repair leaves only an overlong bullet, CareerOS performs a free local rescue: it removes prompt-banned filler or a complete trailing sentence/clause, never slices arbitrary prose, and then reruns the complete editorial gate. The rescue cannot add evidence, numbers, technologies, or claims; any remaining structural, grounding, punctuation, repetition, or unsafe-length defect still fails visibly. Applied actions are stored in `generation_metadata.local_editorial_rescue_actions`. The first rollout label was `original-prompt-v1-quality-gated-local-recovery`; ADR-025 replaces it with the schema-safe audit label used by completed jobs.

**ADR-025** — Schema-safe generation persistence (2026-08-30). Generation version identifiers are audit data stored in `job.generation_version VARCHAR(32)`, so the active deterministic editorial-recovery pipeline is labeled `original-v1-qg-local-recovery` (29 characters) and a contract test prevents future labels from exceeding the persisted schema. The worker's result commit now runs inside its guarded generation block. If any flush or commit fails, the broken transaction is rolled back and the job is reopened under the same RLS identity in a fresh session, then committed as a visible retryable failure. Database persistence defects can no longer escape through an unconditional `finally` and strand a completed ARQ task at `processing`.
